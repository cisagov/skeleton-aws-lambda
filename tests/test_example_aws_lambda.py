...
def test_log_levels(level):
    ...
    with patch.object(sys, "argv", ["bogus", f"--region={TEST_REGION}", f"--message={test_message}", f"--log-level={level}"]):
...
def test_lambda_function_run(capsys):
    ...
    assert (output_lines[0] == f"Region: {TEST_REGION}"), "region did not match what was provided"
    assert (output_lines[1] == f"Invocation Time: {test_time}"), "invocation time did not match what was provided"
    assert (output_lines[3] == f"Provided Message: {test_message}"), "message did not match what was provided"


def test_commandline_run(capsys):
    """Verify that the script works from the commandline."""
    test_message = "pytest-cli_functionality"
    with patch.object(sys, "argv", ["bogus", f"--region={TEST_REGION}", f"--message={test_message}"]):
        eal.main()
    captured = capsys.readouterr()
    output_lines = (captured.out).split("\n")
    assert (len(output_lines) == 5), "unexpected output length"
